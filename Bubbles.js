...
function Bubbles(container, self, options) {
  ...
  animationTime = options.animationTime || 200 // сколько времени занимает анимация чата, также установлена в CSS
  typeSpeed = options.typeSpeed || 5 // задержка на символ, для имитации машинного «набора текста»
  widerBy = options.widerBy || 2 // добавить немного дополнительной ширины к пузырькам, чтобы они не разбились
  sidePadding = options.sidePadding || 6 // отступы пузырьков с обоих сторон чата
  recallInteractions = options.recallInteractions || 0 // количество взаимодействий, которые нужно запомнить и вернуть при перезагрузке
  inputCallbackFn = options.inputCallbackFn || false // мы должны отобразить поле ввода?
  ...
  this.typeInput = function(callbackFn) {
    ...
    inputText.setAttribute("placeholder", "Спроси у меня...")
