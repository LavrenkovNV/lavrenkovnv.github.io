...
if (recognition) {
    ...
    recognition.onresult = function (event) {
        ...
        if (yes_input == 'да') {
            inputString = inputString.replace('да ', '')
        }
        ...
        if (inputString !== null && typeof inputString !== undefined) {
            if (player && inputString.indexOf('pause') != -1) {
                pause_song()
            } else if (player && inputString.toUpperCase().trim() == 'ВКЛЮЧИ') {
                play_song()
        ...
        console.log("Услышал: "+inputString)
    }
...
function response_handler(response) {
    ...
    switch (intent) {
        case 'komplim':
         response_funct = koplim_resp
            break
        case 'song':
         response_funct = play_yt
            break
        case 'rick':
            response_funct = play_yt2
            break
        case 'movie':
            response_funct = load_torrent
            break
            case 'coin':
            response_funct = coinss
            break
        default:
...
function koplim_resp(msg, is_response = false) {
    if (msg.id) {
...
function coinss(msg, is_response = false) {
    push_response('Подкидываю монетку...')
    let output = msg
        if (msg.text) {
            output = msg.text
        }
...
function sendResp(msg){
var xhr = new XMLHttpRequest();
     // xhr.open('GET', 'http://192.168.27.2/speak.php?text=123', false);
      xhr.open('POST', 'http://192.168.27.2:8787/speak.php?text='+msg, true);
      xhr.send();
      if (xhr.status != 200) {
        // обработать ошибку
        ...
        // вывести результат
        console.log('Статус 200. Результат: '+xhr.responseText);
      }
}


//var xhr = new XMLHttpRequest();
//xhr.open('POST', 'http://192.168.27.1/play.php?bvm=/'kiki_agree_b, true)    
//xhr.send();
      


function speak_response(msg, callback) {
//гугл, но они любят ограничители :(  
...
//раньшенужен был на вике,сейчас нет :)
    // sendResp(msg);
}
...
function get_resp(query, is_response = false) {
    //Может выводить перед каждым отображением подождите секунду
